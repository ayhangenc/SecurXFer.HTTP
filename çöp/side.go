...
func readInput1(input chan<- string) {

	for {
		var u string
		fmt.Print("input 1 mesajı ")
		_, err := fmt.Scanf("%s\n", &u)
		if err != nil {
			panic(err)
		}
...
func readInput2(input chan<- string) {

	for {
		var u string
		fmt.Print("input 2 mesajı ")
		_, err := fmt.Scanf("%s\n", &u)
		if err != nil {
			panic(err)
		}
...
func main() {
	...
	userInput1 := make(chan string)
	userInput2 := make(chan string)


	go readInput1(userInput1)
	go readInput2(userInput2)
...
var (
	cRX = make(chan string)
	cTX = make(chan string)
)

func main() {

	fmt.Println("selam dünyalı!..")
...
func f1(cTX chan string) {

	fmt.Println("gönderilecek mesajı giriniz!..")
	mesajTX := f4()

	fmt.Println("mesajTX: ", mesajTX)
...
func f2() (cRX chan string) {

	fmt.Println("alınmış mesajı giriniz!..")
	mesajRX := f4()
	cRX <- mesajRX
	close(cRX)
	return
...
func f3(cRX chan string, cTX chan string) {
...
func f4() (mesajX string) {

	mesajIn := ""
	messageReader := bufio.NewReader(os.Stdin)
	// fmt.Print("Please type your message here: ")
	mesajIn, _ = messageReader.ReadString('\n')
	mesajIn = strings.TrimSuffix(mesajIn, "\n") // remove CR from end
...
func readInput1(input chan<- string, waitGroup *sync.WaitGroup) {
...
func readInput2(input chan<- string, waitGroup *sync.WaitGroup) {
	defer waitGroup.Done()
	for {
		var u string
		fmt.Print("input 2 mesajı ")
...
func main() {
	...
	go readInput1(userInput1, &waitGroup)
	go readInput2(userInput2, &waitGroup)
