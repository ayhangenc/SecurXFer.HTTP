...
func main() {
	...
	for {
		...
		go func() {
			...
			tcpAddr, err := net.ResolveTCPAddr("tcp", fmt.Sprintf("127.0.0.1:%s", "5000"))
			checkError(err)
			...
			handleConnection(connectTo, recvMsg, &wg)
		...
		fmt.Println("Gelen Mesaj - final- : ", msgReceived)
		fmt.Println("Giden Mesaj - final- : ", msgSend)
	}
...
func handleConnection(conn net.Conn, recvMsg chan []byte, wg *sync.WaitGroup) {
...
func client(sendMsg chan []byte, wg *sync.WaitGroup) { // proto string, enc int, ipAddress string, port string, msg string) {
	...
	// }
	messageCRC := crcGenerate(messagetoCRC)
	enc := 1                                                        // CRC Generation
